...
class UserDatabase {
  ...
  async checkIfuserExists(info) {
    try {
      const sqlSelectQuery = `SELECT * FROM user WHERE emailAddress = ? OR mobileNumber = ?`;
  ...
  async userRegistration(info) {
    try {
      const sqlInsertQuery = `INSERT INTO user(fullName, emailAddress, userPassword, mobileNumber) VALUES (?, ?, ?, ?)`;
  ...
  async verifyEmail(emailAddress) {
    try {
      const sqlUpdateQuery = `UPDATE user SET isEmailVerified = 1 WHERE emailAddress = ?`;
